...
print(client.set_device_config("name", 102, firm_client.DeviceProtocol.UART, timeout_seconds=RESPONSE_TIMEOUT))

device_config = client.get_device_config(timeout_seconds=RESPONSE_TIMEOUT)
if device_config:
    print(f"Name: {device_config.name}, Frequency: {device_config.frequency}, Protocol: {device_config.protocol}")