use firm_core::constants::packet_constants::{CRC_SIZE, HEADER_SIZE, IDENTIFIER_SIZE, LENGTH_SIZE, MIN_PACKET_SIZE, PacketHeader};
...
impl MockDeviceHandle {
    fn new(state: Arc<State>) -> Self {
        Self { state, command_buffer: Arc::new(Mutex::new(Vec::new())) }
    }

    /// Waits for a framed command and returns its identifier, or None on timeout.
    pub fn wait_for_command_marker(&self, timeout: Duration) -> io::Result<Option<u16>> {
        ...
        loop {
            ...
            if command_buffer.len() >= MIN_PACKET_SIZE {
                ...
                if command_buffer.len() >= len_start + LENGTH_SIZE {
                    let payload_len = u32::from_le_bytes(command_buffer[len_start..len_start + LENGTH_SIZE].try_into().unwrap()) as usize;
                    let frame_len = HEADER_SIZE + IDENTIFIER_SIZE + LENGTH_SIZE + payload_len + CRC_SIZE;
...
impl MockSerialPort {
    ...
    pub fn pair(timeout: Duration) -> (Box<dyn SerialPort>, MockDeviceHandle) {
        let state = Arc::new(State { timeout: Mutex::new(timeout), ..Default::default() });
        let port: Box<dyn SerialPort> = Box::new(Self { state: state.clone() });
...
impl SerialPort for MockSerialPort {
    ...
    fn baud_rate(&self) -> serialport::Result<u32> { Ok(2_000_000) }
    fn data_bits(&self) -> serialport::Result<DataBits> { Ok(DataBits::Eight) }
    fn flow_control(&self) -> serialport::Result<FlowControl> { Ok(FlowControl::None) }
    fn parity(&self) -> serialport::Result<Parity> { Ok(Parity::None) }
    fn stop_bits(&self) -> serialport::Result<StopBits> { Ok(StopBits::One) }
    fn timeout(&self) -> Duration { self.timeout() }
    fn set_timeout(&mut self, timeout: Duration) -> serialport::Result<()> { *self.state.timeout.lock().unwrap() = timeout; Ok(()) }
    ...
    fn bytes_to_read(&self) -> serialport::Result<u32> { Ok(self.state.device_to_client.lock().unwrap().len() as u32) }
    fn bytes_to_write(&self) -> serialport::Result<u32> { Ok(0) }
    fn clear(&self, _buffer_to_clear: ClearBuffer) -> serialport::Result<()> { Ok(()) }
    fn try_clone(&self) -> serialport::Result<Box<dyn SerialPort>> { Ok(Box::new(self.clone())) }
